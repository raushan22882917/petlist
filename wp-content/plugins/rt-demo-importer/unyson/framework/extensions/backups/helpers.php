...
// added by @reyad
function rt_demo_fw_ext_backups_unzip_partial( $zip, $destination_dir, $last_entry = '', $timeout = 0 ) {
	...
	if ( $zip_object->open( $zip ) !== true ) {
		return new WP_Error(
			'cannot_open_zip', sprintf( __( 'Cannot open zip (Error code: %s)', 'fw' ), $zip_object->status )
		);
	}
	...
	$max_time = time() + $timeout;
	$index    = 0;
	while ( $index < $zip_object->numFiles && time() < $max_time ) {
		$name = $zip_object->getNameIndex( $index );
		if ( substr( $name, - 1 ) === '/' ) {
			++ $index;
			continue; // it is a directory
		}

		if ( $last_entry && $name !== $last_entry ) {
			++ $index;
			continue;
		}

		$destination_path = $destination_dir . '/' . $name;
		if ( ! file_exists( $dest_dir = dirname( $destination_path ) ) && ! mkdir( $dest_dir, 0777, true ) ) {
			$zip_object->close();
		...
		$data = $zip_object->getFromIndex( $index );
		if ( false === ( $unzipped = fopen( $destination_path, 'wb' ) ) ) {
			$zip_object->close();
