<?php
/**
 * Module Styles Trait for Store Categories.
 *
...
trait RTCL_Divi5_StoreCategories_ModuleStylesTrait {

	use RTCL_Divi5_StoreCategories_CustomCssTrait;
	...
	public static function module_styles( $args ) {
		...
		$color_styles = [];
		$item_sel     = "{$order_class} .rtcl-store-cat-item";
		...
		// Grid.
		$color_styles[] = [
			'selector'    => "{$order_class} .rtcl-store-cat-items-wrapper",
			'declaration' => "display: grid !important; grid-template-columns: repeat({$grid_column}, 1fr) !important; gap: 20px !important;",
		];

		// Title.
		if ( $title_color ) {
			$color_styles[] = [ 'selector' => "{$order_class} .rtcl-store-category-title a", 'declaration' => "color: {$title_color} !important;" ];
		}
		if ( $title_hover_color ) {
			$color_styles[] = [ 'selector' => "{$order_class} .rtcl-store-category-title a:hover", 'declaration' => "color: {$title_hover_color} !important;" ];
		}

		// Description.
		if ( $description_color ) {
			$color_styles[] = [ 'selector' => "{$order_class} .rtcl-store-category-description", 'declaration' => "color: {$description_color} !important;" ];
		}

		// Count.
		if ( $count_color ) {
			$color_styles[] = [ 'selector' => "{$order_class} .rtcl-store-category-count", 'declaration' => "color: {$count_color} !important;" ];
		}
			...
			[
				...
				'styles'        => array_merge(
					[
						...
						TextStyle::style( [ 'selector' => "{$order_class} .rtcl-store-categories-wrapper", 'attr' => $attrs['module']['advanced']['text'] ?? [] ] ),
