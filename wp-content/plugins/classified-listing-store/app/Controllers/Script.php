...
class Script {
	...
	function __construct() {
		add_action( 'admin_init', [ $this, 'register_script_for_both' ], 100 );
		add_action( 'admin_init', [ $this, 'register_script_backend' ], 200 );
		add_action( 'admin_enqueue_scripts', [ $this, 'load_admin_script_for_store_post_type' ], 300 );
		add_action( 'admin_enqueue_scripts', [ $this, 'load_admin_script_for_pricing_post_type' ], 300 );
		add_action( 'admin_enqueue_scripts', [ $this, 'load_admin_script_for_settings' ], 300 );
		add_action( 'admin_enqueue_scripts', [ $this, 'load_admin_script_for_manage_membership' ], 300 );
		add_action( 'wp_enqueue_scripts', [ $this, 'register_script_for_both' ], 1 );
		add_action( 'wp_enqueue_scripts', [ $this, 'register_script_frontend' ], 2 );
		add_action( 'wp_enqueue_scripts', [ $this, 'load_script' ], 5 );
	}

	function register_script_for_both() {
		wp_register_script( 'bootstrap-timepicker', RTCL_STORE_URL . '/assets/js/bootstrap-timepicker.js', [ 'jquery' ], '0.5.2', true );
	}
	...
	function register_script_backend() {
		...
		wp_register_script( 'rtcl-store-admin', RTCL_STORE_URL . '/assets/js/admin.js', [
			'jquery',
			'bootstrap-timepicker'
		], $version, true );
		wp_register_script( 'rtcl-admin-pricing', RTCL_STORE_URL . '/assets/js/admin-pricing.js', [ 'jquery' ], $version, true );

		wp_register_style( 'rtcl-membership-app', RTCL_STORE_URL . '/assets/css/membership.app.css', [
			'rtcl-bootstrap',
			'rtcl-admin'
		], $version );
		...
		global $post;
		wp_localize_script( 'rtcl-store-admin', 'rtcl_store', apply_filters( 'rtcl_store_admin_localize', [
			...
			"server_error"          => __( "Server Error.", "classified-listing-store" ),
		] ) );

		wp_localize_script( 'rtcl-membership-app', 'rtcl_store', [
			"ajaxurl"         => admin_url( "admin-ajax.php" ),
	...
	function register_script_frontend() {
		$version = ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? time() : RTCL_STORE_VERSION;
		wp_register_script( 'rtcl-store', RTCL_STORE_URL . '/assets/js/store.js', [
			'jquery',
			'rtcl-validator',
			'bootstrap-timepicker'
		], $version, true );

		wp_register_script( 'rtcl-store-public', RTCL_STORE_URL . '/assets/js/store-public.js', [
			'jquery',
			'rtcl-validator'
		], $version, true );

		wp_register_style( 'rtcl-store', RTCL_STORE_URL . '/assets/css/store.css', [ 'rtcl-public' ], $version );
		wp_register_style( 'rtcl-store-public', RTCL_STORE_URL . '/assets/css/store-public.css', [ 'rtcl-public' ], $version );

		$max_image_size = Functions::get_max_upload();
		global $post;
		wp_localize_script( 'rtcl-store', 'rtcl_store', apply_filters( 'rtcl_store_localize', [
			"store_time_options"    => apply_filters( 'rtcl_store_time_options', [
				"icons" => [
					"up"   => 'rtcl-icon-up-open',
					"down" => 'rtcl-icon-down-open'
			...
			"ajaxurl"               => admin_url( "admin-ajax.php" ),
			'confirm_text'          => __( "Are You sure?", 'classified-listing-store' ),
			rtcl()->nonceId         => wp_create_nonce( rtcl()->nonceText ),
			"max_image_size"        => $max_image_size,
			...
			"server_error"          => __( "Server Error.", "classified-listing-store" ),
			'lng'                   => [
				'error' => __( 'Error while processing data', 'classified-listing-store' )
			]
		] ) );
		wp_localize_script( 'rtcl-store-public', 'rtcl_store_public', apply_filters( 'rtcl_store_public_localize', [
			"ajaxurl"       => admin_url( "admin-ajax.php" ),
			'is_rtl'        => is_rtl(),
			rtcl()->nonceId => wp_create_nonce( rtcl()->nonceText ),
			'store_id'      => is_singular( rtclStore()->post_type ) ? $post->ID : 0
	...
	function load_script() {
		...
		if ( Functions::is_account_page() && ( isset( $wp->query_vars['store'] ) || isset( $wp->query_vars['page'] ) || isset( $wp->query_vars['listings'] ) ) ) {
